...
class Application(QtWidgets.QApplication):
    ...
    def __init__(self) -> None:
        ...
        self.__series: dict[str, list[Series]] = {e.name: [] for e in self.configuration.environments}
    ...
    def register_window(self, window: Window) -> None:
        ...
        if window.IS_MAIN:
            self.series_updated_signal.connect(lambda: window.statusbar.set_left_text(self._get_series_status_text()))

            self.series_retriever.finished_signal.connect(
                lambda: window.statusbar.set_left_text(self._get_series_status_text())
            )
    ...
    def get_series_by_id_or_name(
        self, environment_name: str, series_id: str, series_name: str, warn: bool = True
    ) -> Series | None:
        for series in self.series[environment_name]:
            if series._id == series_id:
                return series

            if series.get_full_name() == series_name or series.name == series_name:
                return series
        ...
        if warn:
            self.notify_user_signal.emit(
                ...
                UI_ERROR_TEXT["series"]["series_not_found_error"]["text"].format(
                    series_id=series_id, series_name=series_name, environment_name=environment_name
                ),
            )
    ...
    def warn_user(self, title: str, text: str) -> None:
        d = WarningDialog(title=title, text=text)
    ...
    def start_task(self, window: Window, description: str, function: Callable, is_generator: bool) -> Worker | None:
        ...
        window.worker = self.worker_controller.run_thread(thread_function=function, thread_is_generator=is_generator)
        ...
        window.worker.about_to_finish_signal.connect(lambda: self.work_ended_signal.emit(window))
