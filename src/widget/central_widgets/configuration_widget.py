...
class MiscTab(BaseWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.add_to_button_group(
            ...
            activity_dict=self.misc.environments_activity,
            row=0,
            col=2,
        )
        ...
        self.add_to_button_group(
            ...
            activity_dict=self.misc.role_activity,
            row=3,
            col=2,
        )
        self.add_to_button_group(
            ...
            activity_dict=self.misc.type_activity,
            row=6,
            col=2,
        )
    ...
    def add_to_button_group(
        ...
        for button_label, is_selected in activity_dict.items():
    ...
    def save_location_button_clicked_handler(self) -> None:
        path = QtWidgets.QFileDialog.getExistingDirectory(self, self.UI_TEXT["sip_creator_save_location_button_text"])

        if not path:
            return

        self.misc.save_location = path
        self.save_location_label.setText(path)

    def bestandscontrole_button_clicked_handler(self) -> None:
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, self.UI_TEXT["bestandscontrole_list_button_text"])

        if not path:
            return

        self.misc.bestandscontrole_lijst_location = path
        self.bestandscontrole_label.setText(path)
