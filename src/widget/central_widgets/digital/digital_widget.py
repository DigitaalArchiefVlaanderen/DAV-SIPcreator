...
class DigitalWidget(CentralWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.sip_databases_locatie_button = QtWidgets.QPushButton(
            text=common_controls["sip_databases_locatie_button_text"]
        )
    ...
    def setup_signals(self) -> None:
        ...
        self.start_sip_button.clicked.connect(
            lambda: self.start_sip_button.button_click_handler(
                selected_dossiers=self.dossier_list_widget.get_selected_items()
            )
        )
        ...
        self.sip_databases_locatie_button.clicked.connect(
            lambda: os.startfile(self.application.configuration.sip_db_location)
        )
    ...
    def dossiers_loaded_handler(self, dossier_paths: list[str]) -> None:
        self.dossier_list_widget.add_widgets(widgets=[DossierWidget(path=p) for p in dossier_paths], select=False)
        ...
        if not self.application.digital_sip_db_controller.is_valid_db(sip.db_name):
            self.application.notify_user_signal.emit(
                ...
                UI_TEXT_ELEMENTS["errors"]["sip"]["invalid_database_error"]["text"].format(
                    db_path=os.path.join(self.application.configuration.sip_db_location, sip.db_name)
                ),
            )
            return
...
class AddDossierButton(QtWidgets.QPushButton):
    ...
    def button_click_handler(self) -> None:
        dossier_path = QtWidgets.QFileDialog.getExistingDirectory(caption=self.UI_TEXT["dialog_message"])
